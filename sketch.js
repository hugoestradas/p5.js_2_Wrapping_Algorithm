...
function setup() {
  ...
  coordenates.sort((a, b) => a.x - b.x);
  leftMost = coordenates[0];
  currentVertex = leftMost;
  hull.push(currentVertex);
  nextVertex = coordenates[1];
  index = 2;
}
...
function draw() {
  ...
  let checking = coordenates[index];
  stroke(255);
  ...
  if (index == coordenates.length) {
    if (nextVertex == leftMost) {
      ...
      index = 0;
      //coordenates.splice(nextIndex, 1);
      nextVertex = leftMost;
    }
  }
}
